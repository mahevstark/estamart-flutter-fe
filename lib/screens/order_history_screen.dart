...
class _OrderHistoryScreenState extends BaseRouteState {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: global.nearStoreModel != null && global.nearStoreModel!.id != null
            ? _isDataLoaded
                ? (orderController.completedOrderList.length > 0) || (orderController.activeOrderList != null && orderController.activeOrderList!.length > 0)
                    ? RefreshIndicator(
                        onRefresh: () async {
                          await _onRefresh();
                        },
  ...
  void initState() {
    ...
    if (orderController.completedOrderList.length > 0) {
