...
class SignUpScreen extends BaseRoute {
  ...
  final int? loginType;
  SignUpScreen({a, o, this.user, this.loginType})
...
class _SignUpScreenState extends BaseRouteState {
  ...
  _onSignUp() async {
    try {
      bool isConnected = await br.checkConnectivity();
      String currentTimeStamp = DateTime.now().microsecond.toString();
      _cPassword.text = currentTimeStamp;
      _cConfirmPassword.text = currentTimeStamp;
      if (isConnected) {
